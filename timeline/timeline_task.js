...
function timeline_task_objects(element, data) {
	...
	function createSVGObject(data) {
	    ...
	    var timelineMarker = createElementSVG('marker', 'time-line-marker', null, {
	        "markerWidth": "10",
	        "markerHeight": "10",
	        "refX": "0.5",
	        "refY": "3",
	    ...
	    var timeline_marker_line = createElementSVG('line', null, 'timeline-marker-line', {
	        ...
	        "marker-end": "url(#time-line-marker)",
	    });
		...
		***/
	    ...
	    for (var i = 0; i < days; i++) {
	        ...
	        mark_object.value = i;
	        // Тут дичь, переписать если понадобится
	    ...
	    function stringToArray(str, len) {
	        /*
			    str - входная строка
			    len - количество символов, на которые разбить
		    */
	        var temp_arr = [];
	    ...
	    function setAnswer(items) {
	        ...
	        for (var i = 0; i < items.length; i++) {
	            ...
	            if (items[i].start_postition) answer_object.start_postition = items[i].start_postition;
	            if (items[i].current_position) answer_object.current_position = items[i].current_position;
	    ...
	    function mouseUpHandler() {
	        if (draggableElement.value === -999) {
	            ...
	            if(draggableElement.current_position){ delete draggableElement.current_position; }
	        }
	        draggableElement = false;
	        // console.log(items);
	        answer.setJSON(setAnswer(items)); 

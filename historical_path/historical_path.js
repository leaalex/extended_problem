function HistoricalPath(settings) {
    ...
    let HistoricalPathInit = {
        ...
        test_filling: function () {
            ...
            cities_array.sort(utils.cy_sorter).forEach((x, index) => x.id = `city_${index + 1}`);
            ...
            for (let i = 0; i < paths.length; i++) {
                ...
                paths[i].onmouseover = function (event) {
                    if (utils.contains_classes(event.target, ["path", "approved"])) {
                        let from = event.target.id.replace("path_","").split("__")[0];
                ...
                paths[i].onclick = function (event) {
                    if (utils.contains_classes(event.target, ["path", "approved", "for-remove"])) {
                        let from = event.target.id.replace("path_","").split("__")[0];
